...
def _generate_reply(question, state, stopping_strings=None, is_chat=False, escape_html=False, for_ui=False):
    ...
    if generate_func is None:
        ...
        if shared.model.__class__.__name__ in ['LlamaCppModel', 'Exllamav2Model', 'CtransformersModel', 'MambaSsmModel']:
...
def encode(prompt, add_special_tokens=True, add_bos_token=True, truncation_length=None):
    ...
    if shared.model.__class__.__name__ in ['LlamaCppModel', 'Exllamav2Model', 'CtransformersModel', 'MambaSsmModel'] or shared.args.cpu:
        return input_ids
...
def get_reply_from_output_ids(output_ids, state=None, starting_from=0):
    ...
    if (hasattr(shared.tokenizer, 'convert_ids_to_tokens') and len(output_ids) > starting_from) and not reply.startswith(' '):
        ...
        if first_token == [None] or first_token.startswith('▁'):
            reply = ' ' + reply
